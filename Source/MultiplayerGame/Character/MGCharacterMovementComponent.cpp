...
const FMGCharacterGroundInfo& UMGCharacterMovementComponent::GetGroundInfo()
{
	...
	else
	{
		...
		const FVector TraceEnd(TraceStart.X, TraceStart.Y, (TraceStart.Z - MGCharacter::GroundTraceDistance - CapsuleHalfHeight));
	
		FCollisionQueryParams QueryParams(SCENE_QUERY_STAT(MGCharacterMovementComponent_GetGroundInfo), false, CharacterOwner);
